...
public:
  ...
  DListNode<T>* insert_after(int key, T d); // insert the int after this node
  // return a pointer to the inserted node
...
template <typename T>
DListNode<T>* DListNode<T>::insert_after(int key,T d) {
  DListNode<T>* temp = new DListNode(key, d);   // 2 OPERATION
